...
首次安装 `3.1.5` 后，Emby 的计划任务页面会出现“Avdb Actor Tools 插件自动更新”。任务安装后
...
读取版本清单时会自动添加缓存绕过参数，避免中间缓存导致任务读取到旧版本。
...
`2.0.1` 及更早版本不包含自动更新任务，必须先手动安装 `3.0.0` 或更高版本。已经安装 `2.1.0`
的实例也必须先手动替换为 `3.0.0` 或更高版本，因为 `2.1.0` 的下载任务存在进度回调缺失问题，
无法自行下载修复版本。
